...
app.get("/api/out-of-stock-items", (req, res) => {
  const query =
    "SELECT itemName, quantity FROM tblitems WHERE quantity < 20 ORDER BY quantity DESC";
  db.query(query, (error, results) => {
    if (error) {
...
app.put("/api/updateProduction/:productionId", (req, res) => {
  ...
  db.beginTransaction(async (transactionErr) => {
    ...
    try {
      ...
      const getUsedMaterialsSql = `SELECT odi.matId, pdu.quantityUsed, mat.matName FROM tblproductionmaterialused pdu LEFT JOIN tblorderfromsupplier_items odi ON pdu.orderItemId = odi.orderItemId LEFT JOIN tblrawmats mat ON mat.matId = odi.matId  WHERE productionId = ?;`;
